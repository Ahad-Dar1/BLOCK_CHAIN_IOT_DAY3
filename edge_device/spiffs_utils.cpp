...
void checkOrGenerateHashKey(const char* Device_name) {
    // Check if a file named "hashkey.txt" exists in SPIFFS.
      char concatenated_file_name[50];
        strcpy(concatenated_file_name,Device_name);
        strcat(concatenated_file_name,"/hashkey.txt");
    if (SPIFFS.exists(concatenated_file_name)) {
        // If the file exists, read its contents.
        readFileFromSPIFFS(SPIFFS, "/hashkey.txt");
    } else {
